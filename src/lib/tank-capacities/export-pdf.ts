...
function sanitizeFilename(title: string): string {
  return (
    title
      .replace(/^Tank Capacities Report — /i, "")
      .replace(/[^\w\-]+/g, "-")
      .replace(/-+/g, "-")
      .slice(0, 40) || "project"
  );
...
export async function exportTankCapacitiesReportPdf(
  data: TankCapacitiesReportData,
): Promise<void> {
  ...
  doc.text(
    "Indicative design estimate — verify tank capacities with class society and project specifications.",
    MARGIN_X,
    y,
  ...
  doc.save(`Tank-Capacities-Report-${name}.pdf`);
}
